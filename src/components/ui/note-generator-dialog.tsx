...
export function NoteGeneratorDialog() {
					...
					onSuccess: (data) => {
						...
						setTimeout(() => {
							setAttachedFiles((prev) =>
								prev.map((f) => (f.name === data.filename && f.status === "uploading" ? { ...f, id: fileId, status: "complete" } : f))
							);
						}, 1000);
					},
					onError: () => {
	...
	const handleGenerateNote = () => {
		...
		const fileIds = attachedFiles.filter((file) => file.status === "complete").map((file) => file.id);
			...
			<DialogContent className="sm:max-w-[500px] bg-surface-container">
				...
				<div className="space-y-6 py-4">
					<div className="space-y-3">
						...
						/>
						<small className="text-secondary">💡 Tip: Be as specific as you'd like. Lex is a great listener.</small>
					</div>
